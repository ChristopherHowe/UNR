...
void my_creatdir(myfs_t* myfs, int cur_dir_inode_number, const char* new_dirname) {
  // all changes to the fs have to use read write paradigm
    // malloc space to read in from the disk
    // use memcpy to copy from myfs->imap to malloced space
    // set bit in malloc space
    // write to myfs->imap using memcpy
  ...
  // set it to used (RWP)
  // Load the parent directory and new directory inodes
  // Increment parent directory size
  // init new directory inode (code provided to init root inode)
    // Note that it should have . and ..
  // update inode table (probably RWP)

  // Access parent directories data and add new entry for new dir(RWP)
    // something about inode.data[0] since only one block
    // should work no matter how many entries parent dir has,
    // can use strlen()

  // modify new directory data to include . and ..(RWP)
