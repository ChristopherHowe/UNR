    ...
    def run(self):
        ...
        if self.train:
            ...
            for episode in range(n_episodes):
                # allow epsilon to slowly decrease over time
                if eps >= eps_end:
                        eps -= 1/eps_decay
                ...
                while done != True:
                    ...
                    #Q update learning rule
                    Q[s, action] += alpha*(reward + gamma * predicted_value - Q[s, action])

                    s = sprime
                

                if episode % 1000 == 0: # Over time alpha gets smaller by a small fraction
                    alpha *= .996
                    print(f"eps: {eps}")            
                if tick < 499:
                    print ("fail " + str(tick) )
                    ...
                    print ("success")
            print(f"average fail: {totalFails/n_episodes}")
            np.save('cart.npy',Q)
